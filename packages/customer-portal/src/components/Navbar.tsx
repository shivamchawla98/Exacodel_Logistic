...
const tools = [
  {
    name: "Logistic Explorer",
    description: " Use our real-time freight calculator to compare rates ",
    href: "/bookings",
    icon: ChartPieIcon,
  },
  {
    name: "Container Tracking",
    description: " See your cargo location on the map in real-time ",
  ...
  },
  {
    name: "Ship Schedules",
    description:
      " Find sailing schedules by route or vessel with multiple shipping lines ",
    href: "#",
    icon: FingerPrintIcon,
  },
  {
    name: "Load Calcultator",
...
export default function Navbar({ navAndFotterHidingRoute }: any) {
  ...
  const hideRoutePresent = navAndFotterHidingRoute.find(
    (item: any) => item == pathname
  );
  const { signUpClicked, sendOtpClicked } = useSelector(
    (state: any) => state.selectForm
  );
  ...
  return (
    <>
      {show && (
        <header className="bg-white border-b-2 border-primary-100">
          <nav
            className=" flex max-w-7xl items-center justify-between py-6 lg:px-6"
            aria-label="Global"
          >
            <div className="flex lg:flex-1 ">
              <Link href="/" className="-m-1.5 p-1.5">
                ...
                <img
                  ...
                  src="https://tailwindui.com/img/logos/mark.svg?color=fuchsia&shade=800"
                  alt=""
                />
              </Link>
            </div>
                      ...
                      {tools.map((item) => (
                        ...
                        >
                          <div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                            <item.icon
                              className="h-6 w-6 text-gray-600 group-hover:text-primary-500"
                              aria-hidden="true"
                            />
                          </div>
                  ...
                  <Popover.Panel className="absolute -left-8 top-full z-10 mt-3 w-96 rounded-3xl bg-white p-4 shadow-lg ring-1 ring-gray-900/5">
                    {company.map((item) => (
                      ...
                      >
                        <Link
                          href={item.href}
                          className="block text-sm font-semibold hover:text-primary-500 leading-6 text-gray-900"
                        >
                          {item.name}
                          <span className="absolute inset-0" />
                        </Link>

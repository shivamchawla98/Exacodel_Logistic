...
import GET_ALL_APPROVED_WAREHOUSE from "@/graphql/query/getAllApprovedWarehouse";
import {
  flexRender,
  getCoreRowModel,
  useReactTable,
...
function ApprovedWarehouseList({
  setActiveItem,
  setApprovalIndex,
  activeItem,
}: any) {
  ...
  const myData = useMemo(
    () => data?.getapprovedWarehouseList ?? [],
    [data?.getapprovedWarehouseList]
  );
  console.log(data?.getapprovedWarehouseList);
  useEffect(() => {
    refetch();
  }, [activeItem]);
  /**
...
export default ApprovedWarehouseList;

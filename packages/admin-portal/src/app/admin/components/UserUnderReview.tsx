...
import GET_WAREHOUSE_REVIEW_LIST from "@/graphql/query/getReviewWarehouse";
import {
  flexRender,
  getCoreRowModel,
  useReactTable,
...
function UserUnderReviewWarehouse({
  setActiveItem,
  setApprovalIndex,
  activeItem,
}: any) {
  ...
  const { loading, error, data, refetch } = useQuery(GET_WAREHOUSE_REVIEW_LIST);
  ...
  const myData = useMemo(
    () => data?.getWarehousereveiwedlist ?? [],
    [data?.getWarehousereveiwedlist]
  );
  console.log(data?.getWarehousereveiwedlist);
  useEffect(() => {
    refetch();
  }, [activeItem]);
  /**
    ...
    <div className="inline-block max-w-4xl py-2 align-middle sm:px-6 lg:px-8">
      <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
        <div className="relative my-4 max-w-xl">
          <input
            type="text"
            className="border rounded-ee-md rounded-tr-md border-gray-300 rounded-md px-3 py-2 w-full focus:outline-none focus:ring focus:border-primary-500 text-sm text-gray-700 placeholder-gray-400"
...
export default UserUnderReviewWarehouse;

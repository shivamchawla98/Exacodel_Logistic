...
const validationSchema = Yup.object().shape({
  ...
  minimumStorageRent: Yup.string().required(
    "Minimum Storage Rent per Sq Ft is required"
  ),
  minimumStorageCharges_per_pallet: Yup.string().required(
    "Minimum Storage Charges per Pallet is required"
  ),
  minimumStorageArea: Yup.string().required(
    "Minimum Storage Area per Sq Ft is required"
  ),
  minimumstorageArea_per_pallet: Yup.string().required(
    "Minimum Storage Area per Pallet is required"
  ),
  storageChargesPerSqFt: Yup.string().required(
    "Storage Charges per Sq Ft is required"
  ),
        ...
        variables: {
          input: {
            ...
            minimumStorageCharges_per_pallet:
              values.minimumStorageCharges_per_pallet * 1,
  ...
  return (
    <>
      {showPopUp && (
        <ApprovedPopup
          name={companyName}
          operation="Warehouse Added"
          onApprovalClick={() => {
            setShowPopUp(false);
            setActiveItem("My Warehouses");
          }}
        />
      )}
      <ToastContainer />
